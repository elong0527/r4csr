...
Lastly, we will provide reusable templates for updating ADRG and ARM,
...
The demo project (R package) we will prepare for submission is
available on GitHub at
[elong0527/esubdemo](https://github.com/elong0527/esubdemo).
...
The files in two directories within the eCTD folder structure
are critical for documenting analysis using R: `datasets`
and `programs`. We assume the eCTD folder is `esub`.
...
Folder path: `esub/analysis/adam/datasets/`.

- ADaM data in `.xpt` format: created by SAS
...
Folder path: `esub/analysis/adam/programs/`.

- `r0pkgs.txt`: contains all internally developed R packages
...
submission package. We will focus on preparing the internal
R packages in the following sections.
...
syntax or the
[remotes dependency syntax](https://remotes.r-lib.org/articles/dependencies.html).
...
[startup script](https://github.com/elong0527/esubdemo/blob/master/inst/startup.R)
when the R project is opened.
...
evaluates a specified scope of folders and files defined by
a list of **file specifications**, and generate a **file collection** object.
...
knitr::purl(
  ...
  output = "esub/analysis/adam/programs/tlf-01-disposition.txt"
)
```

Repeat this for all the R Markdown files.
...
we should update the ADRG (ADaM Reviewer's Guide) to provide
