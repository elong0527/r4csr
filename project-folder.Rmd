...
- Consistency: everyone work on the same folder structure.
...
We recommend using the R package folder structure to organize analysis scripts 
...
Consistent folder structure also reduces communication cost between study team members 
...
We refer to an R package as a **standard R package** if the purpose of an R package is to 
share commonly used R functions to be hosted in a code repository such as CRAN. 
...
> Here the `adam\` folder contains ADaM datasets. 
> The `tlf` folder contains output TLFs in RTF format.
> We put `adam\` and `tlf\` folder within the `vignettes` folder only for illustration purposes. 
> In a real A&R report, you may have a different location to save your input and output.
...
The `renv` provides a robust and stable approach to manage R package versions 
...
The R code above in the `.Rprofile` initiate the `renv` running environment.  
