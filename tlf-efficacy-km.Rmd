...
par(mfrow = c(1, 1),
    mar = c(9, 11, 1, 1),
    family = "Times") # Set plot margins and font style
...
grid <- 0:6
mtext(
  c("Placebo", risk_tab(grid, adtte_ttde$AVAL_m[adtte_ttde$TRTP == "Placebo"])),
  side = 1,
  adj = c(1, rep(NA, length(grid))),
  line = 5,
  at = c(-0.8, grid)
)
mtext(
  c("Xanomeline Low Dose",
    risk_tab(grid, adtte_ttde$AVAL_m[adtte_ttde$TRTP == "Xanomeline Low Dose"])),
  side = 1,
  adj = c(1, rep(NA, length(grid))),
  line = 6,
  at = c(-0.8, grid)
...
mtext(
  c("Xanomeline High Dose",
    risk_tab(grid, adtte_ttde$AVAL_m[adtte_ttde$TRTP == "Xanomeline High Dose"])),
  side = 1,
  adj = c(1, rep(NA, length(grid))),
  line = 7,
  at = c(-0.8, grid)
...
ggsave("tlf/fig_km2.png",
       plot = fig_km2,
       width = 10,
       height = 6)

# Create RTF figure
rtf_read_png("tlf/fig_km2.png") %>% # Read PNG files from the file path
  rtf_title(
    "Kaplan-Meier Plot for Time to First Dermatologic Event by Treatment Group",
    "All Participants"
  ) %>% # Add title or subtitle
...
```{r}
# Estimate KM data point
surv_mod <-
  visR::estimate_KM(data = adtte_ttde,
                    strata = "TRTP",
                    AVAL = "AVAL_m")
# save plot with white background and Times New Roman font
...
```{r, out.width = "100%", out.height = "400px", echo = FALSE, fig.align = "center"}
knitr::include_graphics("tlf/fig_km3.pdf")
```
