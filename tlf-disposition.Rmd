...
Following [ICH E3 guidance](https://database.ich.org/sites/default/files/E3_Guideline.pdf), we need to summarize the accounting of all patients who entered the study, in Section 10.1, Disposition of Patients.
...
For a disposition table, all the required information is saved in the ADSL dataset.
We can use the `haven` package to read the dataset.
...
In the code below, we calculate the number and percentage of patients who discontinued the study.
...
In the code below, we calculate the number and percentage of patients who completed/discontinued the study for different reasons.
...
In the code below, we calculate the number and percentage of patients who complete the study.
...
In the code below, we calculate the number and percentage of patients who discontinued the study for different reasons.
...
Now we combined individual rows into the whole table for reporting purposes.
`tbl_disp` is used as input for `r2rtf` to create the final report.
...
We highlighted items that is not discussed in the previous discussion.
...
-   Step 4: Count the number and percentage of patients who discontinued the study in different reasons, and name the dataset as `n_reason`. Format it using `r2rtf`.
-   Step 5: Count the number and percentage of patients who completed the study, and name the dataset as `n_complete`.
-   Step 6: Bind `n_rand`, `n_disc`, `n_reason` and `n_complete` by row. Format it using `r2rtf`.
